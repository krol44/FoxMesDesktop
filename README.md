...
FoxMes Desktop 1.4.9 is distributed without a commercial code-signing certificate. Before bypassing an operating-system warning, verify the downloaded file against `SHA256SUMS` from the same GitHub release.
...
```powershell
Unblock-File .\FoxMes-1.4.9-windows-x64-setup.exe
```

For the portable version, unblock the ZIP before extracting it:

```powershell
Unblock-File .\FoxMes-1.4.9-windows-x64-portable.zip
Expand-Archive .\FoxMes-1.4.9-windows-x64-portable.zip .\FoxMes-portable
...
```bash
chmod +x FoxMes-1.4.9-linux-x86_64.AppImage
./FoxMes-1.4.9-linux-x86_64.AppImage
```

If AppImage mounting is unavailable, extract and run it directly:

```bash
./FoxMes-1.4.9-linux-x86_64.AppImage --appimage-extract
...
```bash
mkdir FoxMes
tar -xJf FoxMes-1.4.9-linux-x86_64.tar.xz -C FoxMes
./FoxMes/AppRun
```
