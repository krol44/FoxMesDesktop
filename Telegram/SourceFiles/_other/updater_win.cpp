		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"FoxMes.exe") && exeName != L"FoxMes.exe") {
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF && versionNum != 0x7FFFFFFE) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{F65B4EBE-8E1B-58C8-AED1-B3E8E207EA5C}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								wsprintf(nameStr, L"FoxMes Desktop");
								...
								wsprintf(icongroupStr, L"FoxMes Desktop");
...
static const WCHAR *_programName = L"FoxMes Desktop"; // folder in APPDATA, if current path is unavailable for writing
