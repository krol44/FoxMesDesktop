# FoxMes Desktop 1.8.2 validation
...
Before publishing `v1.8.2`, complete these manual checks on the produced artifacts:
...
- macOS 13+ on Apple silicon: mount the DMG, launch the app, check that `codesign -d -r- /Applications/FoxMes.app` names the `codesign.pem` certificate, that Screen Recording, Camera and Microphone stay granted after updating over the previous release, verify `foxmes://open`, notifications, autostart and coexistence. Intel Macs are out of scope: the package is arm64-only.
...
- Endpoint isolation: a package build launched with `FOXMES_URL=http://attacker.invalid` still uses `https://foxmes.foxtail.ing` and does not disable TLS verification.

Version 1.8.2 is unsigned. Record SmartScreen/Gatekeeper behavior in the release checklist and verify every downloaded artifact against both `SHA256SUMS` and GitHub provenance.
